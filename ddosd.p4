...
control MyIngress(inout headers hdr,inout metadata meta,inout standard_metadata_t standard_metadata) {
    ...
    apply {
        ...
        //Drop packets when device recives alarm and its alarmed state
        if (hdr.ethernet.ether_type == 0x0800 && meta.first_device == 1 && meta.attack == 1 ){
            drop();
        } else {
                    ...
                    if (current_ow == 1) {
                        ...
                        if (current_ow > training_len_aux) {
                            ...
                            if ((meta.src_entropy << 14) > src_thresh || (meta.dst_entropy << 14) < dst_thresh){
                                meta.alarm = 1;
                                device_status.write(0,meta.alarm);
                            }
        ...
        if (hdr.ipv4.isValid())
            ...
            if (standard_metadata.instance_type == CLONE) {
                if (meta.key == 0) {
                    ...
                    hdr.ipv4.total_len = hdr.ipv4.total_len + 30;
