...
struct metadata {
    bit<16> parser_count; /* Number of headers for parser */
    ...
    bit<16> ib_position; /* Header stack position to read ip address in alarm packet */
    bit<32> trigib; /* Trigger when alarm packet with ip addresses is received */
    bit<32> ib_address; /* Address to include in IP Blocker */
    bit<32> ib_index; /* IP Blocker index when write*/
    ...
    apply {
        ...
        if(meta.ib_source == meta.ib_read){ /* meta.ib_source == meta.ib_read */
            ...
            if (hdr.ipv4.isValid() && meta.alarm_pktin != 1) {
                ...
                //Adding IP address received into IPBlocker
                if (hdr.ddosd.isValid()){
